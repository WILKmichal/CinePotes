...
export class AuthController {
  ...
  async register(@Payload() body: RegisterDto) {
    ...
    const user = await this.usersService.createUser(nom, body.email, body.password, role);
  ...
  async forgotPassword(@Payload() data: { email: string; expiresInMinutes?: number }) {
    const expiresInMinutes = data.expiresInMinutes ?? 30;

    const token = await this.usersService.issuePasswordResetToken(data.email, expiresInMinutes);

    if (token) {
      const frontUrl =
        process.env.FRONT_RESET_PASSWORD_URL ?? 'http://localhost:3000/reset-password';
  ...
  async resetPassword(@Payload() body: { token: string; newPassword: string }) {
    const ok = await this.usersService.resetPasswordWithToken(body.token, body.newPassword);
